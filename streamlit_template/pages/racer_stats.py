...
df_unp_racer = df_single_racer.unstack().rename_axis(['catagory','row number']).reset_index().drop(columns='row number').rename({0:"strength"},axis =1)
# st.write(df_unp_racer)
st.bar_chart(df_unp_racer,x='catagory', y='strength')
